...
# Shared theme integration: ornotto

## Ornotto documentation with FontLab editorial chrome

`src_docs/properdocs.yml`, `src_docs/overrides/` and site assets build the book at https://fontlab.org/ornotto/. The book now uses the global FontLab menu/footer and shared Marketing-style editorial components; its content, navigation and benchmarks remain Ornotto-owned.
...
|---|---|---|
| `src_docs/properdocs.yml` | active publication configuration outside sibling checkout root | https://fontlab.org/ornotto/ |

## Build and publish responsibility

Use the documentation workflow in this repository; build `src_docs/properdocs.yml` with its configured environment and publish `docs/` through the existing Pages workflow. Do not rebuild or release the native decision engines for a documentation-only change.
