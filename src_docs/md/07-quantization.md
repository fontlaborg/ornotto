...
One query is 1.5 points of accuracy on a 67-query set, so a difference of one or two answers between neighbouring quants is noise. If you read the grid expecting a smooth curve, you will find bumps that do not repeat: Qwen3.5-2B scores 61 at `q3` and 57 at `q8`, which says more about three borderline queries than about 3-bit weights.
...
For the 0.8B and 1.7B models, `q5` is the lowest quant that keeps the full score, and on pcdServer it is often the best row of its model:
...
The gain of `q5` over `q8` is two answers, inside the noise, so the practical reading is that `q5` loses nothing. The `q4` files of the same models lose one to three answers, and `q3` loses more.
...
|---|---|---|---|---|
| 0.5–0.6B | Qwen3-0.6B `q8`, pcdServer | 0.64 | 53/67 | 25.5 |
| 0.8B | decider-0.8b `q8`, dohnuts (Metal) | 0.81 | 62/67 | 51.4 |
| 1.7–2B | Qwen3.5-2B `q3`, pcdServer | 1.22 | 61/67 | 42.7 |
...
- **3B to 4B models**: `q4`. It costs at most one answer against `q8` and cuts the download from 4.48 GB to 2.71 GB for Qwen3.5-4B-Hmm. The Qwen3.5-4B files keep their score even at `q2`; Qwen3-4B at `IQ1_S` does not.
...
The registered models in the `ornotto` package follow this: `decider-0.8b`, `decider-2b`, `kev-0.8b` and `qwen3.5-0.8b` are `q8`, the 2B and 4B chat models are `q4` ([chapter 10](10-package.md)).
