...
jev, the hosted reference, answered 65 of the 67 router queries correctly in both modes, at 466 ms per query. The best local model came within one query of it: Qwen3.5-4B-Hmm on pcdServer scored 64/67 on translated text and 65/67 on direct text, at 88 ms per query from a 4.5 GB file, or 93 ms from the 2.7 GB Q4_K_M file. The dedicated decider-0.8b on dohnuts (Metal) scored 62/67 and 61/67 at 51 ms from 0.8 GB, and the laya encoder scored 52/67 in 7 ms. 208 runs finished; two failed and are left out.

## The top of the table

The fifteen best runs, ordered by translated accuracy, then direct accuracy, then speed:

--8<-- "tables/top.html"

Five findings stand out.

- **A small, dedicated model gets within three queries of jev.** decider-0.8b answers 62 of 67 through its own readout, from a file a fifth the size of the Hmm model's Q8 build. It is the fastest run to score 62 or more, and its probabilities are calibrated.
- **A tuned 4B chat model on pcdServer matches the 35B decider.** Qwen3.5-4B-Hmm (64/65) scores the same as decider-35b-a3b on dohnuts (64/64). Its 2.7 GB Q4 build is an eighth the size of the 35B file and runs three times faster. Its accuracy does not move from Q4 to BF16.
...
- **Size does not rescue an old model.** The best Qwen1.5 run, at 4B, scored 42/67 in 248 ms. Qwen3.5 at 0.8B scored 57.
...
All 208 runs are in the table below. Click a column header to sort by it, and click again to reverse the order. Type in the box to keep only the rows that contain every word you type: `pcdServer q4` keeps the pcdServer runs at Q4, and `dedicated Metal` keeps the dedicated models on dohnuts (Metal). [Chapter 5](05-method.md#what-the-numbers-mean) defines every column.
...
- **Translation helps weak multilingual models and costs strong ones nothing.** kev-0.8b scored 57 translated and 45 direct, Qwen2.5-3B at Q8 62 and 57, laya 52 and 49. The Hmm model scored one point better without translation, 65 against 64, and so did the Qwen3.5-4B builds.
...
- **Two runs failed.** A Core ML laya export fixed at 64 tokens and four options could not take a five-way question. The BF16 build of unsloth's Qwen3.5-4B was not on disk when its turn came. Neither appears in the tables.

## Where each engine lands

Most runs are pcdServer runs, because it takes any chat GGUF: 156 of the 208. The other engines ran only the models they were built for. Each engine's best run:

| engine | runs | best run | translated / direct | ms/query |
|---|---:|---|---|---:|
...
If you need an answer in under 10 ms on a Mac, only laya delivers it, at 52/67. If you can spend 40 to 55 ms, pcdServer with a 2B chat model or dohnuts with decider-0.8b reaches 61 or 62. Every run above 62 took at least 85 ms.
...
Reading a dedicated model through its own readout is worth several points on small models. For a fine-tuned or vanilla chat model, pcdServer's chat-template readout is the one that works: the Hmm model scored 64/65 there and 62/63 through its letter readout on llama-server, and in a third of the time.
