...
DATA = HERE / "data"
OUT = HERE / "md" / "tables"
QUANT_ORDER = ["iq1", "q1", "q2", "iq2", "q3", "iq3", "q4", "iq4", "q5", "q6", "q8", "int8", "w8", "fp16", "f16",
               "bf16", "fp32"]
TASKS = 67
...
def classifier_rows(rows: list[dict]) -> list[str]:
    ...
    for r in rows:
        out.append("".join([
            cell(r["method"], cls="method"), cell(r["engine"]), cell(r["model"]), cell(r["family"]), cell(r["quant"]),
            num(r["gb"], 2), num(r["ms"], 1), score(r["translated"]), score(r["direct"]),
            score(r["translated_en"], 28), score(r["translated_other"], 39), score(r["direct_other"], 39),
            num(r["load_ms"]),
        ]))
    return out


CLASSIFIER_HEADERS = ["Method", "Engine", "Model", "Family", "Quant", "GB", "ms/query", "Translated", "Direct",
                      "English", "Other, translated", "Other, direct", "Load ms"]
...
def main() -> None:
    ...
    same = [r for r in rows if r["model"] in ("decider-0.8b", "decider-0.8b-dreamblooms", "decider-2b",
                                              "decider-2b-dreamblooms", "decider-35b-a3b", "decider-4b")]
    same.sort(key=lambda r: (r["model"], r["quant"], r["ms"]))
    table("engines-same-model", ["Model", "Quant", "Engine", "ms/query", "Translated", "Direct"],
          ["".join([cell(r["model"]), cell(r["quant"]), cell(r["engine"]), num(r["ms"], 1), score(r["translated"]),
                    score(r["direct"])]) for r in same], filterable=True)
    ...
    table("queries", ["Query", "Lang", "Task", "Right, translated", "Right, direct", "jev"],
          ["".join([cell(q["query"], cls="query"), cell(q["lang"]), cell(q["task"]),
                    num(q["translated_share"] * 100, 0, " %"), num(q["direct_share"] * 100, 0, " %"),
                    cell(q["jev_translated"] or "")]) for q in hard], filterable=True)

    table("translator", ["Query", "Lang", "English", "ms"],
          ["".join([cell(t["query"], cls="query"), cell(t["lang"]), cell(t["english"], cls="query"), num(t["ms"], 0)])
           for t in load("translator")], filterable=True)
    table("detectors", ["Detector", "Correct", "Mean µs", "Calls/s"],
          ["".join([cell(d["detector"]), score(d["correct"]), num(d["mean_us"], 1), num(d["calls_per_s"], 0)])
           for d in load("detectors")])

    cascade = load("cascade")
    for mode in ("translated", "direct"):
        c = cascade[mode]
        table(f"cascade-{mode}", ["Gate: top probability below", "Fallbacks", "Correct", "Mean ms"],
              ["".join([num(s["threshold"], 2), cell(f"{s['fallbacks']}/{c['n']}", s["fallbacks"], "num"),
                        score(s["correct"]), num(s["ms"], 1)]) for s in c["sweep"]])
        buckets = [("<0.6", "below 0.6"), ("0.6-0.8", "0.6 to 0.8"), ("0.8-0.95", "0.8 to 0.95"), (">=0.95", "0.95 and up")]
        table(f"calibration-{mode}", ["Top probability", "Answers", "Right", "Share right"],
              ["".join([cell(label), num(c["calibration"][k][0]), num(c["calibration"][k][1]),
                        num(100 * c["calibration"][k][1] / c["calibration"][k][0], 0, " %")])
               for k, label in buckets if k in c["calibration"]])
